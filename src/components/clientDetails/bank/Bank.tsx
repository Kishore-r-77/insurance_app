...
import { addApi, deleteApi, editApi, getAllApi } from "./bankApis/bankApis";
...
function Bank({ modalFunc, bankClntData, lookup }: any) {
  ...
  const reducer = (state: BankStateType, action: any) => {
    switch (action.type) {
  ...
  const getData = () => {
    return getAllApi(pageNum, pageSize, state)
      .then((resp) => {
        setData(resp.data["All Banks"]);
        settotalRecords(resp.data.paginationData.totalRecords);
        setisLast(resp.data["All Banks"]?.length === 0);
