...
function WithdrawnScrModal({ open, handleClose, policyId }: any) {
  ...
  const onChange = (e: any) => {
    ...
    setWithdrawnData({ ...WithdrawnData, [name]: value });
  };

  const onChangeReqDate = (date: any) => {
    setWithdrawnData({ ...WithdrawnData, RequestedDate: date });
  };
    ...
    <div>
      <Modal show={open} onHide={handleClose} centered size="xl">
        <Modal.Header closeButton>
          <Modal.Title>{"Withdrawn"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <form>
            <Grid2 container spacing={2}>
              ...
              <Grid2 xs={8} md={6} lg={12}>
                <TextField
                  ...
                  value={WithdrawnData.ReasonDescription}
                ...
                <FormControl style={{ marginTop: "0.5rem" }} fullWidth>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DesktopDatePicker
                      ...
                      value={WithdrawnData.RequestedDate}
                      onChange={(
...
export default WithdrawnScrModal;
