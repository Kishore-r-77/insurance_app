...
function PostponeScrModal({
  ...
  const [PostponeData, setPostponeData] = useState(initialValues);
  const onChange = (e: any) => {
    const { value, name } = e.target;
    setPostponeData({ ...PostponeData, [name]: value });
  };

  const onChangeReqDate = (date: any) => {
    setPostponeData({ ...PostponeData, RequestedDate: date });
  };
    ...
    <div>
      <Modal show={open} onHide={handleClose} centered size="xl">
        <Modal.Header closeButton>
          <Modal.Title>{"Postpone"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <form>
            <Grid2 container spacing={2}>
              ...
              <Grid2 xs={8} md={6} lg={12}>
                <TextField
                  ...
                  value={PostponeData.ReasonDescription}
...
export default PostponeScrModal;
