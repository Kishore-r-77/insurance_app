...
function IBenefitModal({
  ...
  const aprroveib = () => {
    axios
      .post(
        `http://localhost:3000/api/v1/customerservice/ibapprove/${IBapprovebenefit.PolicyID}/${IBapprovebenefit.BenefitID}`,
        {},
  ...
  return (
    <div>
      <CustomIBenefitModal
        ...
        title={title}
        isSave={isSave}
        handleFormSubmit={saveibenefitOpen} /////
          ...
          <TreeItem nodeId="2" label={`Benefits`}>
            <Paper className={styles.paperStyle}>
              <Table striped bordered hover style={{ position: "relative" }}>
                ...
                {IBenefits?.map((val: any, index: number) => (
                  <tr>
                    <td>
                      <input
